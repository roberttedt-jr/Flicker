...
// ==========================================
// 1. CONFIGURACIÓN (API REAL & MOCK ROBUSTO)
// ==========================================
...
// Géneros fijos para las filas estilo Netflix
...
// DATOS DE RESPALDO (Para cuando no hay internet)
const MOCK_DB = [
  { 
...
// ==========================================
// 2. ESTILOS "LIQUID GLASS" REUTILIZABLES
// ==========================================
...
// Componente para aplicar estilos CSS globales y ocultar scrollbars
const AppContainer = ({ children }) => (
    <div className="relative min-h-screen bg-[#050505] font-sans text-gray-100 overflow-hidden selection:bg-blue-500/30 selection:text-white">
        {/* CSS NATIVO PARA OCULTAR BARRAS DE DESPLAZAMIENTO EN NAVEGADORES WEBKIT/FIREFOX */}
        <style>{`
            /* Ocultar barra de desplazamiento para WebKit (Chrome, Safari, Opera) */
            .scrollbar-hide::-webkit-scrollbar {
                display: none;
            }
            /* Ocultar barra de desplazamiento para IE and Edge */
            .scrollbar-hide {
...
export default function App() {
  ...
  const [movies, setMovies] = useState([]); // Usado solo para búsqueda
  const [genreSections, setGenreSections] = useState({}); // Nuevo estado para las filas
  ...
  // --- DATA FETCHING (CON DEBOUNCE Y FILTROS) ---
  
  // Efecto principal para cambiar pestañas (que dispara la carga de filas)
  useEffect(() => {
    if (activeTab === 'search') {
      setMovies([]);
      setGenreSections({});
  ...
  const fetchContent = async (category, query = '') => {
    ...
    try {
      ...
      if (category === 'search') {
        ...
        setMovies(filtered);
        setGenreSections({}); // Limpiamos secciones en búsqueda
        setFeaturedMovie(null);
        return;

      } else {
        // 2. Lógica de Filas de Género (Netflix Style)
        const genresToFetch = category === 'movies' ? POPULAR_MOVIE_GENRES : POPULAR_TV_GENRES;
        
        // --- 2.A. Cargar Tendencia Global (Destacado) ---
        ...
        // --- 2.B. Cargar Fila de Populares en General (Nueva Sección) ---
        ...
        // Añadir Fila de Populares en General como primera sección
        sectionsMap[9000] = { 
        ...
        // --- 2.C. Cargar Filas por Género (concurrentemente) ---
        ...
        setMovies([]); // Aseguramos que el estado de 'movies' (de búsqueda) esté vacío
      }
      ...
      const mockSections = {
        9000: { name: "Populares del Día (Demo)", data: MOCK_DB }, // Nueva fila de Populares
    ...
    <AppContainer>
      ...
      <div className={`relative z-10 transition-all duration-700 ease-out ${view !== 'app' ? 'scale-95 blur-xl opacity-40 pointer-events-none grayscale-[0.3]' : 'scale-100 blur-0 opacity-100'}`}>
        ...
        <main className="pt-32 pb-32 px-4 md:px-12 max-w-[1600px] mx-auto min-h-screen">
          ...
          {/* Home Content (Movies/Series) */}
          {(activeTab === 'movies' || activeTab === 'series') && (
            <div className="animate-in fade-in duration-700">
              
              {/* ELEMENTO DESTACADO (Siempre toma el top trend general) */}
              ...
              {/* FILAS DE GÉNEROS (Estilo Netflix) */}
              {loading ? (
                ...
                <>
                {/* --- NUEVA SECCIÓN: POPULARES EN GENERAL (ID 9000) --- */}
                ...
                {/* --- FILAS POR GÉNERO (Resto de IDs) --- */}
                {Object.keys(genreSections)
                    .filter(id => id !== '9000') // Excluir Populares, ya lo mostramos
                    .map(genreId => {
          ...
          {/* My List Tab (Old logic - kept for tab view) */}
          {activeTab === 'mylist' && (
...
const ContentRow = ({ section, myList, setSelectedMovie, usingBackup, icon }) => (
    <div className="mb-12 animate-in slide-in-from-right duration-700">
        ...
        {/* Si la lista está vacía, mostramos un mensaje dentro de la estructura de fila */}
        {section.isEmpty ? (
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // 2. TRAILERS (Intento ES -> Fallback EN)
